...
When a selected branch uses Archify, store optional `vNN-archify.<type>.json`, `vNN-archify.html`, `vNN-archify.svg`, and `vNN-archify-validation.json` alongside the candidate, plus its pinned upstream revision and semantic object mapping. Their hashes enter the candidate provenance chain, but a successful Archify schema/layout check never substitutes for the editable-PPTX inspection, visual-object trace, reader, semantic, or slide gates. Do not package the Archify self-contained HTML as the final deck asset unless the user separately requests it.

Before `package`, run `pipeline.py verify-package --archive <zip> --deck <package-relative-deck> --render-manifest <package-relative-manifest> --output <json>`. The archive root must contain those package-relative paths directly; do not wrap the payload in a top-level staging directory. It rejects encrypted, duplicate, symlink, traversal, oversized, and high-compression ZIP members before bounded clean extraction, then revalidates the manifest, slide hashes/dimensions, and declared producer assets. A packaged review trail is not reproducible until that check passes.
