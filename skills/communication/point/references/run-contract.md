...
Hard checks: claim_ids_preserved, citations_and_qualifiers_preserved, critical_terms_defined_or_known, actors_and_actions_explicit, paragraphs_answer_one_reader_question, language_is_natural_for_audience, unresolved_literal_translation_count=0, and security_boundary_explanation_required (boolean). When a boundary explanation is required, security_boundary_explanation_complete must be true: the text must name the role, the received or produced artifact, the validation, the decision or allowed action, and the malicious, stale, or unavailable-input outcome. This review may improve wording and information order, but it must not add a claim, weaken a condition, delete an evidence boundary, or replace technical precision with vague plain language.
