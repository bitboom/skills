...
1. Read references/run-contract.md, references/baseline-and-selection.md, references/component-catalog.md, references/component-selection.md, references/delivery-modes.md, references/prose-preservation.md, and references/rubrics.md.
2. Require one passed canonical Point package: `point.md`, `point.yaml`, `point.sha256`, and `point-gate.json`. Verify that all hashes agree. Return to Point for missing or disputed meaning; To Deck does not research new claims.
3. A derivative JSON/YAML projection may accelerate layout only when it carries the canonical Point hash and a material-ID projection manifest. Read canonical `point.md` as the prose source. A projection with fewer material Point IDs is selective input, never a complete structural source. On an actor, artifact-direction, boundary, or outcome disagreement, fail closed and return to Point.
4. Run `scripts/pipeline.py init` and resolve audience, venue, language, template, accessibility, prior-deck constraints, and `delivery_profile`: `summary`, `structural`, or `paired`. `summary` is one or two slides; `structural` is a complete Point deck; `paired` produces both from the same frozen Point hash. Record the production command, source script, and lockfile when the deck is programmatically generated.
5. For a teaching vehicle, keep the domain as title and dominant visual. Limit project assessment to a small supporting lens.
...
## Build the selected delivery profile
...
2. Before storyboarding, make a `prose-trace.json` from canonical `point.md`. For every material claim, qualifier, proof limit, failure state, developer implication, and executive implication, identify the reader-visible sentence that will carry it. A label, ID chip, source footer, or diagram arrow cannot satisfy this row.
3. For every non-appendix slide, write an assertion-led title, a subtitle that answers the reader question, and at least one readable narrative sentence. For a security boundary, make the chain explicit: actor → concrete artifact → check → distinct policy owner → action or failure. Follow `references/prose-preservation.md` for Korean wording and local term onboarding.
4. For `summary`, preserve the one/two-slide hierarchy: domain takeaway; dominant causal diagram; proof limit and decision/action; small source-project lens only when Point requires it. Freeze `summary_must_see_ids` and every deliberate omission. A summary is not an incomplete claim of full Point coverage.
5. For `structural`, build a navigable complete Point deck. Before build and after inspected PPTX render, require `structural-coverage-map.json` to map every material Point claim, model node/edge/boundary/condition, source reference, qualifier, proof/non-proof, caveat, failure state, and implication to a readable slide and editable object. Group by semantic cluster; do not reduce full coverage to prose bullets, notes, or a separate site.
6. For `paired`, build the summary and structural decks as independent visual products from one immutable Point input. Produce `summary-structural-crosswalk.json`; it maps every summary must-see ID to its structural slide/object and rejects contradictory actor, artifact, direction, proof/non-proof, decision-owner, or outcome semantics. See `references/delivery-modes.md`.
7. Use a renderer-backed technical-diagram prototype only after a visual grammar is selected. Archify may implement an architecture, workflow, sequence, data-flow, or lifecycle candidate from a pinned revision, but it never chooses the grammar or substitutes for Point/visual-model/reader gates. Record its typed IR, version/revision, validation/check output, and rendered SVG hash; treat self-contained HTML or raster export as review material, not the final PowerPoint component. Recreate every meaning-bearing final object as editable PowerPoint content and map it to selected IDs. See `references/component-selection.md`.
8. Use the Presentations skill. Keep important objects editable and map them to selected component IDs, visual-baseline IDs, Point Claim IDs, and model IDs. Add no new material claims.
9. Render every slide with `scripts/render_deck.py` at full size and thumbnail size. The helper clears stale render outputs, requires the PPTX/PNG page counts to agree, produces PDF, PNG, extracted text, and a schema-v2 relative-path manifest with renderer and producer provenance. A script-produced deck must include its source hash and includes a lockfile hash when a lockfile governs the build; a manual deck must say so explicitly.
...
1. **Semantic Diagram Reviewer:** verify canonical Point coverage, causal closure, actor separation, direction, boundaries, and proof limits from the rendered slide. It must compare the canonical Point inventory with any build projection and reject omitted material IDs or conflicting routes.
...
4. **Technical Writer:** reject meta-evaluation headlines, unexplained critical terms, vague labels, and compression that removes the mental model. For every critical security relation, verify a reader-visible Korean sentence names actor, artifact, check, decision owner, and action/failure—not merely a diagram label or footer.
